...
- **Supabase changes**: Whenever a new migration (`supabase/migrations/`), Edge Function (`supabase/functions/`), or schema change is added, deploy it to the remote Supabase project. Use `supabase db push` for migrations and `supabase functions deploy <function-name>` for Edge Functions (the CLI is installed at `~/.supabase/bin/supabase`). If `SUPABASE_ACCESS_TOKEN` or `SUPABASE_SERVICE_ROLE_KEY` are unavailable, document the required deploy steps for the user rather than skipping deployment.
