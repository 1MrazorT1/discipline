...
- **Local builds**: `scripts/build-apk.sh` builds a debug APK locally (portable JDK + Android SDK in `.local-build/`). No sudo or system-wide SDK install required.
- **Agent behavior**: Before editing, confirm target file with user. Reference exact paths (`app/(app)/index.tsx`, `lib/meals.ts`, etc.).
- **Commits**: Make meaningful, targeted commits grouped by concern (feat, test, build, chore). Never `git add .` as a blob. Run `npm test` and `npm run typecheck` before every commit.
